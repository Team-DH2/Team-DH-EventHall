...
export default function EventHallsPage({
  ...
  const applyFilters = () => {
    ...
    if (capacity) {
      filtered = filtered.filter((hall) => {
        const [minCap, maxCap] = hall.capacity.split("-").map(Number);
    ...
    <div>
      <aside className="w-full md:w-[400px] bg-[#1F2024] text-gray-200 rounded-2xl p-6 shadow-lg border border-[#2f2f36] relative">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-2 bg-[#2A2B2F] p-2 rounded">
        ...
        <div className="flex gap-3 mt-6">
          <button
            ...
            className="bg-blue-600 text-white py-3 rounded-xl w-full hover:bg-white hover:text-black transition-all"
          >
            Apply Filters
          </button>
          ...
          <button
            ...
            className="border border-blue-600 text-blue-400 py-3 rounded-xl w-full hover:bg-white hover:text-black transition-all"
          >
            Reset
          </button>
        </div>
